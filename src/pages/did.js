export const idlFactory = ({ IDL }) => {
  ...
  const HttpStreamingCallbackToken__1 = IDL.Record({
    'key' : IDL.Text,
    'sha256' : IDL.Opt(IDL.Vec(IDL.Nat8)),
    'index' : IDL.Nat,
    'content_encoding' : IDL.Text,
  });
  const HttpStreamingCallbackResponse__1 = IDL.Record({
    'token' : IDL.Opt(HttpStreamingCallbackToken__1),
    'body' : IDL.Vec(IDL.Nat8),
  });
  const HttpStreamingStrategy__1 = IDL.Variant({
    'Callback' : IDL.Record({
      'token' : HttpStreamingCallbackToken__1,
      'callback' : IDL.Func(
          [HttpStreamingCallbackToken__1],
          [HttpStreamingCallbackResponse__1],
          ['query'],
        ),
    }),
  });
  const HttpResponse__1 = IDL.Record({
    ...
    'headers' : IDL.Vec(HeaderField__1),
    'upgrade' : IDL.Bool,
    'streaming_strategy' : IDL.Opt(HttpStreamingStrategy__1),
  ...
  return IDL.Service({
    ...
    'http_request_update' : IDL.Func([HttpRequest__1], [HttpResponse__1], []),
